...
/* =========================================================
   FAVORITES — LOCAL TEST MODE
   ========================================================= */

/**
 * IMPORTANT
 *
 * En localhost uniquement :
 *
 * - les favoris sont stockés dans sessionStorage ;
 * - aucun INSERT/DELETE Supabase n'est effectué ;
 * - les données sont temporaires pour le test ;
 * - aucun changement de .env.local n'est nécessaire.
 *
 * En production :
 * - le fonctionnement Supabase normal est conservé.
 */

const LOCAL_FAVORITES_STORAGE_KEY =
...
function getLocalTestFavorites(): string[] {
  if (
    !isLocalhostFavoriteTest()
  ) {
    return [];
  }
...
function saveLocalTestFavorites(
  templateIds: string[],
): void {
  if (
    !isLocalhostFavoriteTest()
  ...
  try {
    ...
    console.error(
      "[Smart Point] local favorite storage:",
      error,
    );
  }
}
    ...
    queryFn:
      ...
      > => {
        ...
        if (
          ...
          if (error) {
            console.error(
              "[Smart Point] local favorites templates:",
              error,
            );
